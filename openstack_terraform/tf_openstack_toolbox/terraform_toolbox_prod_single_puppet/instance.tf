...
resource "openstack_compute_instance_v2" "toolbox" {
  ...
  #create a update_host script from template
  resource "local_file" "puppet_script_file" {
      content = templatefile("${path.module}/templates/puppet_update.tpl", {
        node_name = format("%s.%s", var.name, var.domain_name)
        })
      filename = "${path.module}/scripts/update_puppet.sh"
  }
  ...
  resource "null_resource" "scripts" {
    ...
    # run puppet one last time
        provisioner "local-exec" {
      command = "sleep 60; bash ./scripts/update_puppet.sh"
    }
}