...
resource "openstack_compute_instance_v2" "toolbox" {
  ...
  availability_zone = format("zone%d", (count.index % 1) + 1)
...
//   }
  ...
  resource "null_resource" "scripts" {
    ...
    provisioner "local-exec" {
      command = "sleep 120; ansible-playbook -i ${openstack_compute_instance_v2.toolbox.network.0.fixed_ip_v4}, ansible/ldap/deploy_ldap.yaml"
    }
//     #run jira playbook
//     provisioner "local-exec" {
